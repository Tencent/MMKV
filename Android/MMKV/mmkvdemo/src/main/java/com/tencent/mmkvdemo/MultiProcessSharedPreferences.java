					...
					mReceiver = new BroadcastReceiver() {
						...
						public void onReceive(Context context, Intent intent) {
							...
							if (mName.equals(name) && keysModified != null) {
								...
								List<SoftReference<OnSharedPreferenceChangeListener>> listeners = new ArrayList<SoftReference<OnSharedPreferenceChangeListener>>(arrayListeners);
			...
			case COMMIT:
				...
				for (Map.Entry<String, Object> entry : values.valueSet()) {
					...
					if (v instanceof EditorImpl || v == null) {
						...
						if (hasListeners && map != null && (!map.containsKey(k) || (map.containsKey(k) && !v.equals(map.get(k))))) {
							keysModified.add(k);
						}
					}
